...
func TestFSMStart(t *testing.T) {
	...
	f1 := func(sctx context.Context, s State, opts ...StateOption) (State, error) {
		...
		args := s.Body().(map[string]any)
		if v, ok := args["request"].(string); !ok || v == "" {
			return nil, fmt.Errorf("empty request")
		}
		return &state{name: "", body: map[string]any{"response": "state1"}}, nil
	}
	f2 := func(sctx context.Context, s State, opts ...StateOption) (State, error) {
		...
		return &state{name: "", body: map[string]any{"response": "state2"}}, nil
	}
	f3 := func(sctx context.Context, s State, opts ...StateOption) (State, error) {
		...
		return &state{name: StateEnd, body: map[string]any{"response": "state3"}}, nil
	}
	f.State("1", f1)
	f.State("2", f2)
	f.State("3", f3)
	rsp, err := f.Start(ctx, map[string]any{"request": "state"})
	if err != nil {
		t.Fatal(err)
	}
	args := rsp.(map[string]any)
	if v, ok := args["response"].(string); !ok || v == "" {
